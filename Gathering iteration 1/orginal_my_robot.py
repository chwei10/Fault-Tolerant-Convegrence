...
class Robot:
    ...
    #     filtered_robots = []
    #     for robot in robots:
    #         if self.id != robot.id and self.is_in_observation_range(robot):
    #             filtered_robots.append(robot)
    #     return filtered_robots
    # Return a list of robots that are in the observation range of the robots
    def observe(self, robots):
        return [robot for robot in robots if (self.id != robot.id and self.is_in_observation_range(robot))]
    
    # Identify if the condition of gathering has fullfiled or not, if so terminate the program
    def identify_gathering(self, robots, threshold):
        for robot in robots:
            if self != robot:
                if self.get_distance(robot) > threshold:
                    return False
        return True
    ...
    def move(self, robots_visible, robots):
        if len(robots_visible) == len(robots) - 1:
            ...
            for robot in robots_visible:
                if(self.get_distance(robot) > 0.5):
                    GATHERING = False
                    print("Not gathering!")
                    break
            if GATHERING:
        ...
        # if len(robots_visible) == len(robots) - 1:
        #     if self.identify_gathering(robots,0.1):
        #         # GATHERING = True
        #         self.terminate = True
        #         print("Gathering!")
        #         return
        ...
        center_x, center_y, _ = smallestenclosingcircle.make_circle([(robot.x, robot.y) for robot in robots_visible])
        print("Center: ", center_x, center_y)
        # For each robot, calculate the distance of Lj and choose the minimum one as LIMIT
        Lj_list = []
        for robot in robots_visible:
            cosine, sine = self.trigonometrics((center_x, center_y), (robot.x, robot.y))
        ...
        LIMIT = min(Lj_list)  
        GOAL = math.dist((self.x, self.y), (center_x, center_y))
        ...
        destination_x = self.x + MOVE * (center_x - self.x) / math.dist((self.x, self.y), (center_x, center_y))
        destination_y = self.y + MOVE * (center_y - self.y) / math.dist((self.x, self.y), (center_x, center_y))
